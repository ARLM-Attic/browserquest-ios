...
define(['jquery', 'storage'], function($, Storage) {
    ...
    var App = Class.extend({
        init: function() {
            ...
            window.plugins.analytics.startTrackerWithAccountID("UA-*******");
            window.plugins.analytics.trackPageView("Start");
            
            
            // After device ready, authenticate with success and fail callbacks.
            window.gameCenter.authenticate(
                function(result) {
                    //alert(result);
                },
                function(result) {
            ...
            window.gameCenter.onshow = function() {
                $('#achievementsbutton').addClass("active");
            }
            window.gameCenter.onhide = function() {
        ...
        initXPBar: function () {
            var self = this;
            var scale = this.game.renderer.getScaleFactor(),
                xpMaxWidth = 202 * scale;

            $("#xppoints").css('width', 0 + "px");

            this.game.onPlayerXPChange(function (xp, maxXp) {

                if (self.game && self.game.player && self.game.player.getLevel() < Types.getMaxLevel()) {
                    var percent = (xp / maxXp);
                    var barWidth = Math.round((xpMaxWidth / maxXp) * (xp > 0 ? xp : 0));
                    $("#xppoints").css('width', barWidth + "px");

                    if (percent > 0.993) {
                        $("#xppoints").css('border-top-right-radius', 3 * scale + "px");
                    } else {
                        $("#xppoints").css('border-top-right-radius', "0px");
                    }
                } else {
                    //Hide XP on full level
                    $("#xppoints").css('width', "0px");
                    $("#xppoints").css('border-top-right-radius', "0px");
                }
        ...
        toggleAchievements: function() {
            ...
            window.gameCenter.isAuthenticated(function(isOk) {
                if (isOk) {
                    window.gameCenter.showAchievements();
                } else {
                    $('#achievementsbutton').removeClass("active");
                    window.gameCenter.authenticate( function(result) {
                        window.gameCenter.showAchievements();
                    },
                    function(result) {
                        alert("Did noit auth. Go to Game Center and login.");
                        $('#achievementsbutton').removeClass("active");
                    } );
                }
            }, function() {});
        ...
        showMessage: function (message, priority) {
            var self = this;
            var $wrapper = $('#notifications div'),
                $message = $('#notifications #message2');

            var messagePriority = priority || 0;
            if (messagePriority >= this.currentMessagePriority) {
                this.currentMessagePriority = messagePriority;
