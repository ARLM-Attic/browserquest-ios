...
function(Camera, Item, Character, Player, Timer) {
    ...
    var Renderer = Class.extend({
        init: function(game, canvas, background, foreground, textcanvas, toptextcanvas) {
            ...
            this.tablet = true; //Detect.isTablet(window.innerWidth);
        ...
        rescale: function(factor) {
            ...
            if(window.innerWidth <= 1000) {
                this.imagescale = 2; //iPhone
            } else {
                this.imagescale = 1; //iPad is scaled
            }
            ...
            if(!this.upscaledRendering && this.game.map && this.game.map.tilesets) {
                this.setTileset(this.game.map.tilesets[this.imagescale - 1]); //this.setTileset(this.game.map.tilesets[this.scale - 1]);
            }
            if(this.game.renderer) {
                this.game.setSpriteScale(this.imagescale);
            }
        },
        ...
        createCamera: function() {
            ...
            this.canvas.width = this.camera.gridW * this.tilesize * this.imagescale;
            this.canvas.height = this.camera.gridH * this.tilesize * this.imagescale;
        ...
        drawCellRect: function(x, y, color) {
            this.context.save();
            this.context.lineWidth = 2*this.imagescale;
            this.context.strokeStyle = color;
            this.context.translate(x+2, y+2);
            this.context.strokeRect(0, 0, (this.tilesize * this.imagescale) - 4, (this.tilesize * this.imagescale) - 4);
            this.context.restore();
        },

        drawCellHighlight: function(x, y, color) {
            var s = this.imagescale,
                ts = this.tilesize,
                tx = x * ts * s,
                ty = y * ts * s;
        ...
        drawSelectedCell: function() {
            var sprite = this.game.cursors["target"],
                ...
                os = this.upscaledRendering ? 1 : this.imagescale,
                ds = this.upscaledRendering ? this.imagescale : 1;
        ...
        drawScaledImage: function(ctx, image, x, y, w, h, dx, dy) {
            var s = this.upscaledRendering ? 1 : this.imagescale;
        ...
        drawEntityName: function(entity) {
            if (entity.name && entity instanceof Player) {
                ...
                if (entity.team !== Types.Team.NEUTRAL) {
                    color = (entity.team === Types.Team.RED) ? "#cf7c6a" : "#0085E5";
                }
                    
                this.drawText(this.textcontext, entity.name + " (" + entity.level + ")",
        ...
        */
        ...
        setCameraView: function(ctx) {
            ctx.translate(-this.camera.x * this.imagescale, -this.camera.y * this.imagescale);
        },
